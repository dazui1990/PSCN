...
(function (factory) {
    ...
    tText = '<textarea class="kv-preview-data file-preview-text" title="{caption}" readonly ' + STYLE_SETTING +
        '>{data}</textarea>\n';
   /* tVideo = '<OBJECT width="304" height="100" id="MediaPlayer1"><embed class="kv-preview-data" src="{data}" width="{width}" height="{height}" type="{type}"  autostart="true" loop="false" ></embed></OBJECT>\n';
    */
    tVideo = '<video  width="{width}" height="{height}"  controls autoplay>'+
    '<source src="{data}" type="video/mp4">'+
    '<source src="{data}" type="video/ogg">'+
    '<source src="{data}" type="video/webm">'+
    '<object data="{data}" width="{width}" height="{height}" >'+
    '<embed src="{data}" width="{width}" height="{height}" >'+
    '</object></video>';
   /* tVideo = '<video width="320" height="240" controls autoplay>'+
    '<source src="{data}" type="">' + DEFAULT_PREVIEW + '</video>';*/
...
//        progress: tProgress, /* tofix 原有样式*/
    ...
    defaultFileTypeSettings = {
        ...
        video: function (vType, vName) {
            return compare(vType, 'video.*') && (compare(vType, /(ogg|mov|mp4|mp?g|webm|3gp)$/i) ||
                compare(vName, /\.(og?|mov|mp4|webm|mp?g|3gp)$/i));
        },
    ...
    FileInput.prototype = {
        ...
        _setFileDropZoneTitle: function () {
            ...
            if ($zone.find('.file-preview-frame').length === 0 && isEmpty(self.defaultPreviewContent)) {
            	if(self.$container.find('.file-caption-main').css('display') === 'none'){
            		title = "无可查看的附件";
            	}
