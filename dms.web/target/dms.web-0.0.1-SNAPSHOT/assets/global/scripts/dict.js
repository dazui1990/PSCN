	...
	function initCheckBoxValue(valueArray,checkBoxObj,valueName,descName,checkBoxContainer){
		...
		if(displayStyle!=undefined&&displayStyle=="vertical"&&dictLabelName!=undefined){
			...
			if(isShowallSel){
				...
				htmlStr="<label class='checkbox-title ' ><input type='checkbox' hidden='true' name='"+name+"_all' id='"+(id)+"_all' ><span>"+dictLabelName+"</span></label>";
...
//		$(checkBoxObj).children().remove();
	...
	function initRedio(radioBoxObj){
		...
		if(model){
			...
			//初始化Redio Value
			if(dictDataMap||$(radioBoxObj).attr("uiData")){
				var dictValueArray;
				if(dictDataMap){
					...
					dictValueArray= JSON.parse($(radioBoxObj).attr("uiData"));
				}
	 ...
	 */
	function initSelectOneOption(selectObj,optionValue,valueName,descName,excluedeItems,incluedItems){
		if(incluedItems!=undefined){
			if(incluedItems.indexOf(optionValue[valueName])>=0){
				var optionDesc;
	...
	function initSelectValue(valueArray,selectObj,valueName,descName){
		...
		var incluedItems = $(selectObj).attr("data-includeItems");
		//展示下拉框值
		if(valueArray){
			if($.isArray(valueArray)){
				$.each(valueArray, function(j, value) {
					//初始化某一个option值
					initSelectOneOption(selectObj,valueArray[j],valueName,descName,excluedeItems,incluedItems);
				});
			}else  if (typeof (valueArray)=="object"){
				for(var key in valueArray){
					//初始化某一个option值
					initSelectOneOption(selectObj,valueArray[key],valueName,descName,excluedeItems,incluedItems);
			    }  
			}
		}
	...
	function modeUrl(selectObj){
		...
		if(cacheData != undefined){
			...
			// 如果存在UI data,则使用ui data
			if($(selectObj).attr("uiData")){
				var uiDataValue = JSON.parse($(selectObj).attr("uiData"));
				initSelectValue(uiDataValue,selectObj,labelValue,labelDesc);
			}else{
				//发起ajax 请求
				dmsCommon.ajaxRestRequest({
					url : dmsCommon.getDmsPath()[model] + url,
	...
	function initSelectData(selectObj){
		if($(selectObj).attr("data-model")){
			...
			if(dictDataMap||$(selectObj).attr("uiData")){
				...
				if(type=="dict"){
					if(dictDataMap){
						...
						dictValueArray = dictData[dictCode];
					}else if($(selectObj).attr("uiData")){
						dictValueArray= JSON.parse($(selectObj).attr("uiData"));
					}
	...
	function getParentData(dictValueArray,selectObj){
		var container = $(selectObj).closest(dmsCommon.DMS_CLOSEST_DIV);
		var parentArray = new Array();
		var obj = selectObj;
		while($(obj).attr("parent")){
			var parentObj = $("#"+$(obj).attr("parent"),container);
			var parentValue = $(parentObj).val();
			if(parentValue){
				parentArray.splice(0,0,parentValue);
			}
			obj = parentObj;
		}
	...
	function bindSelectParentEvent(selectObj,rootSelect,container){
		...
		//绑定parent 事件
		if($("select[parent='"+selectId+"'],input[type='checkbox'][parent='"+selectId+"']",container).size()>0){
			
			//checkBox 的下拉框进行暂存
			$.each($("input[type='checkbox'][parent='"+selectId+"']",container),function(i,childObj){
				//暂存checkBoxObj
				if($(selectObj).data("changeCheckBoxParent")==undefined){
					$(selectObj).data("changeCheckBoxParent",$(childObj).parent());
				}
				//暂存checkBoxObj
				if($(selectObj).data("changeCheckBoxObj")==undefined){
					$(selectObj).data("changeCheckBoxObj",childObj);
				}
				//先隐藏掉
				$(childObj).hide();
			});
			...
			$(selectObj).off("change").on("change",function(){
				...
				//补充checkbox 到checkBox parent
				if($(selectObj).data("changeCheckBoxObj")&&$(selectObj).data("changeCheckBoxParent")){
					$($(selectObj).data("changeCheckBoxParent")).html($(selectObj).data("changeCheckBoxObj"));
				}
				
				$.each($("select[parent='"+selectId+"'],input[type='checkbox'][parent='"+selectId+"']",container),function(i,childObj){
					...
					//执行动态查询
					selectChangeEvent(childObj,rootCascadeData,selectObj,container);
				});
			});
	...
	//动态多级关联清空子节点
	function cleanSelectChildObj (obj,container){
		var selectId = $(obj).attr("id");
		if(selectId){
			var childObj = $("select[parent='"+selectId+"']",container);
			if(childObj){
				initSelectValue(null,$(childObj));
				cleanSelectChildObj(childObj,container);
			}
		}
	}
	/**
	 * 动态三级联动
	 */
	var selectChangeEvent = function(childObj,selectParentData,selectObj,container){
		//清空子节点
		//initSelectValue(null,$(childObj));
		cleanSelectChildObj (childObj,container);
		var initFlag = false;
		...
		if(initFlag){
			...
			}else if($(childObj).attr("type")=="checkbox"){
				initCheckBox(childObj);
			}else if($(childObj).attr("type")=="radio"){
				initRedio(childObj);
			}
...
//			return ($("label.checkbox-inline",$(obj).parent().parent()).size()>0);
	 ...
	 * 根据codeIds 获取code 数据
	 */
	var getCodeDatasByIds = function(codeType,codeIds,splitCode){
		if(isStringNull(codeIds)){
			return undefined;
		}else{
			var splitCodeStr = splitCode==undefined?",":splitCode;
			var codeIdArray = codeIds.split(splitCodeStr);
			var codeList = dictDataMap["dict"][codeType];
			//获取符合ID 描述  
			var codeObjectArray = $.grep(codeList, function(item,index){
				  return $.inArray(item.code_id+"", codeIdArray) > -1;
			});
	 ...
	 * 根据codeIds 获取code 数据
	 */
	var getCodeDatasByType = function(codeType){
		if(isStringNull(codeType)){
			return undefined;
		}else{
			return dictDataMap["dict"][codeType];
		}
	}
	...
	/**
	 * 根据type 获取所有小类 170602by侯飞 魔改
	 */
	var getCodesByType = function(type, codeType){
	...
	return {
        ...
        initCheckBoxValue:function(valueArray,checkBoxObj,valueName,descName,checkBoxContainer){
            initCheckBoxValue(valueArray,checkBoxObj,valueName,descName,checkBoxContainer);
		},
        initRadioValue:function(valueArray,radioBoxObj,valueName,descName){
            initRadioValue(valueArray,radioBoxObj,valueName,descName);
		},
		getCodeDescById:function(type,codeType,codeId){
			return getCodeDescById(type,codeType,codeId);
		},
		//根据tc_code 类型获取code 值，splitCode 不是必填的，如果没填，默认为","
		getCodeDatasByIds:function(codeType,codeIds,splitCode){
			return getCodeDatasByIds(codeType,codeIds,splitCode);
		},
		//根据tc_code 类型获取 全部 code值
		getCodeDatasByType:function(codeType){
			return getCodeDatasByType(codeType);
		},
		//刷新URL下拉框
		refreshSelectByUrl:function(selectObj,urlData){
			selectChangeEvent(selectObj,urlData);
		},
		//刷新下拉框的值，使用tc_code 对象
		refreshSelectByCodeData:function(selectObj,valueArray){
			dmsDict.refreshSelectByData(selectObj,valueArray,"code_id","code_cn_desc");
		},
		//刷新下拉框的值by数据
		...
		getCodesByType:function(type,codeType){
			return getCodesByType(type,codeType);
		}
