...
var dmsValidate = function() {
	...
	var initForm = function(formObj){
		$(formObj).validate({
            ...
            errorPlacement: function(error, element) {
            	//显示错误信息
            	showErrorTip(error,element,'show',formObj);
            }
        });
	};
	 ...
	 * 显示错误信息
	 */
	function showErrorTip(error,element,method,formObj){
		var tipContainer;
		if($(element).attr("type")=="checkbox"){
			...
			tipContainer = $("div.bootstrap-tagsinput",$(element).closest("div"));
		}else{
		...
		if(error&&!isStringNull(error.text())&&method=='show'){
			if($(tipContainer).is(":hidden")){
				var returnHiddenType = showHiddenValidateForm(tipContainer,formObj);
				if(returnHiddenType=="tab"){
					
					setTimeout(function(){
						//显示错误信息
						showValidateTip(tipContainer,error);
					},200);
					return;
				}
			}
	 ...
	 * 显示具体的错误信息
	 */
	function showValidateTip(tipContainer,error){
	 ...
	 * 打开隐藏的显示范围
	 */
	function showHiddenValidateForm(tipContainer,formObj){
		var hiddenDiv;
		if($(hiddenDiv=$(tipContainer).closest("div.tab-pane:hidden",formObj)).size()>0){
			var tabId = $(hiddenDiv).attr("id");
			$(">ul >li a[href='#"+tabId+"']",$(hiddenDiv).closest("div.tabbable-custom")).click();
			return 'tab';
		}
		if($(hiddenDiv=$(tipContainer).closest("div.panel-body:hidden",formObj)).size()>0){
			$(hiddenDiv).show();
			return 'panel';
		}
	}
	...
	var bindContainerEvent = function(container,isForm){
		...
		//对金额进行处理
		$("input.money,input.decimal",container).on("click",function(event){
			var moneyElement = $(this);
			if(parseFloat($(moneyElement).val())==0){
				$(moneyElement).attr("data-orginalMoeny",$(moneyElement).val());
				$(moneyElement).val("");
			} 
		});
		//对金额进行处理
		$("input.money,input.decimal",container).on("blur",function(event){
			var moneyElement = $(this);
			if(isStringNull($(moneyElement).val())){
				$(moneyElement).val($(moneyElement).attr("data-orginalMoeny"));
			} 
		});
	};
...
//			onkeyup: null,
	 ...
	 * 限制最小值,不包括等于
	 */
	$.validator.addMethod( "maxGT", function(value, element, param ) {
		value = $.trim(value);
		if(this.optional( element )){
			return true;
		}
		return parseFloat(value) < parseFloat(param);
	}, $.validator.messages.maxGT );
	...
	$.validator.addMethod( "money", function(value, element) {
		...
		//进行格式化转换
		if($(element).attr("data-number-format")){
			value = $.parseNumber(value, {format:$(element).attr("data-number-format"),locale:"us"})+"";
		}
...
//		}
	...
	$.validator.addMethod("posInteger", function(value, element) {
	    ...
	    return true;   
	}, $.validator.messages.posInteger );
	
	/**
	 * 比对数值大小
	 */
	$.validator.addMethod("equalPartQuantity", function() {
		var old = $("#returnQuantity").val();
		var newold = $("#returnPartQuantity").val();
		if(old<newold){
	        return false;  
	    }
	    return true;   
	}, $.validator.messages.equalPartQuantity );
	...
	$.extend( $.validator.messages, {
		...
		maxGT: $.validator.format( "请输入小于 {0} 的数值"),
		...
		passWD:function(parematers,element){
			return $.validator.format( "长度至少6位;", parematers );
		},
		telePhone:function(parematers,element){
			return $.validator.format( "请输入正确的电话号码", parematers );
		},
		mobilePhone:function(parematers,element){
			return $.validator.format( "请输入正确的手机或电话号码", parematers );
		},
		...
		posInteger:function(parematers,element){
			return $.validator.format( "请输入大于0的整数", parematers );
		},
		thousandth:function(parematers,element){
			return $.validator.format( "请输入正确的金额", parematers );
		},
		equalPartQuantity:function(parematers,element){
			return $.validator.format( "退料数量不能大于可退料数量", parematers );
		},
	});
});
